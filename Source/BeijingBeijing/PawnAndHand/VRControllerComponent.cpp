...
void UVRControllerComponent::SendRenderTransform_Concurrent()
{
...
void UVRControllerComponent::updateHandPosition( FVector &Position )
{
	const AActor *TempActor = GetOwner();
	const AVRCharacterBase *Player = Cast<AVRCharacterBase>( TempActor->GetOwner() );

	if ( Player && !Player->VRCamera->IsUpdatePosition )
	{
		FVector cameraPos = Player->VRCamera->GetHMDInputLocation();
		Position -= FVector( cameraPos.X , cameraPos.Y , 90.0f );
	}
}

//=============================================================================
void UVRControllerComponent::TickComponent( float DeltaTime , enum ELevelTick TickType , FActorComponentTickFunction *ThisTickFunction )
{
...
//=============================================================================
bool UVRControllerComponent::PollControllerState( FVector& Position , FRotator& Orientation , float WorldToMetersScale )
{
	if ( IsInGameThread() )
	{
...
//=============================================================================
void UVRControllerComponent::FViewExtension::BeginRenderViewFamily( FSceneViewFamily& InViewFamily )
{
...
//=============================================================================
void UVRControllerComponent::FViewExtension::PreRenderViewFamily_RenderThread( FRHICommandListImmediate& RHICmdList , FSceneViewFamily& InViewFamily )
{
	if ( !MotionControllerComponent )
	{
		return;
...
void UVRControllerComponent::FViewExtension::GatherLateUpdatePrimitives( USceneComponent* Component , TArray<LateUpdatePrimitiveInfo>& Primitives )
{
